...
describe('Index page', () => {
  ...
  describe('Cart page', () => {
    it('returns correct status code when id is a number', (done) => {
      request('http://localhost:7865/cart/123', (err, result, text) => {
        expect(err).to.be.null;
        expect(result.statusCode).to.equal(200);
    ...
    it('returns 404 status code when id is NOT a number', (done) => {
      request('http://localhost:7865/cart/not-a-number', (err, result, text) => {
        expect(err).to.be.null;
...
describe('/available_payments endpoint', () => {
  it('returns available payment methods', (done) => {
    request('http://localhost:7865/available_payments', { json: true }, (err, result, text) => {
      expect(err).to.be.null;
      expect(result.statusCode).to.equal(200);
